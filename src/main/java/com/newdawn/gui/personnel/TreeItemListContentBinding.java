 ...
 */
public class TreeItemListContentBinding<S> implements ListChangeListener<S>,
		WeakListener {
		...
		if (localList == null) {
			...
			while (paramChange.next()) {
				if (paramChange.wasPermutated()) {
					localList.subList(paramChange.getFrom(),
							paramChange.getTo()).clear();
				...
				if (paramChange.wasRemoved()) {
					localList.subList(
							paramChange.getFrom(),
							paramChange.getFrom()
									+ paramChange.getRemovedSize()).clear();
				}
				if (paramChange.wasAdded()) {
					final List<? extends S> listToConvert = paramChange
							.getAddedSubList();
					localList.addAll(paramChange.getFrom(),
							convertList(listToConvert));
				}
			}
		}
	}
