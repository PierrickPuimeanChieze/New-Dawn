...
public class ColonyEconomicScreen implements Initializable {
	...
	public void initialize(URL fxmlFileLocation, ResourceBundle resources) {
		...
		totalPopulationLabel.textProperty().bind(
				new PopulationBinding(colonyProperty(), "population"));
		agriculturePopulationLabel.textProperty()
				.bind(new PopulationBinding(colonyProperty(),
						"agriculturePopulation"));
		servicePopulationLabel.textProperty().bind(
				new PopulationBinding(colonyProperty(), "servicesPopulation"));
		industryPopulationLabel.textProperty().bind(
				new PopulationBinding(colonyProperty(), "industryPopulation"));

		annualGrowRateLabel
				.textProperty()
				.bind(format(
						"%.2s%%",
						selectFloat(colonyProperty(), "populationGrowRate")
								.multiply(
										config.getPopulationGrowRateAnnualMultiplicator())));
	}
}
