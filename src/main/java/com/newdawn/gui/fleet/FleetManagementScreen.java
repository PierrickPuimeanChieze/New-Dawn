...
public class FleetManagementScreen implements Initializable {
	...
	public ObjectProperty<Squadron> squadronProperty() {
		if (squadronProperty == null) {
			...
			squadronProperty.addListener(new ChangeListener<Squadron>() {
				@Override
				public void changed(ObservableValue<? extends Squadron> arg0,
						Squadron arg1, Squadron arg2) {
	...
	private ObjectProperty<StellarSystem> stellarSystemProperty() {
		if (stellarSystemProperty == null) {
			stellarSystemProperty = new SimpleObjectProperty<>(this,
					"stellarSystem");
			ObjectBinding<StellarSystem> select = Bindings.select(
					squadronProperty(), "contextualStellarSystem");
			stellarSystemProperty.bind(select);
			stellarSystemProperty
					.addListener(new ChangeListener<StellarSystem>() {
						@Override
						public void changed(
								ObservableValue<? extends StellarSystem> arg0,
								StellarSystem arg1, StellarSystem arg2) {
							LOG.debug("stellar System changed from" + arg1
									+ " to " + arg2);
						}
					});
		}
	...
	private void initPlottedOrdersListView() {
		plottedOrdersListView.setCellFactory(new PropertyListCellFactory(
				"shortDescription", null));
		ObjectBinding<ObservableList<Order>> select = Bindings.select(
				squadronProperty(), "plottedOrders");
		plottedOrdersListView.itemsProperty().bind(select);
	}

	private void initAvailableOrders() {
		availableOrdersListView
				.setCellFactory(new PropertyListCellFactory<OrderFactory>(
						"name", null));
		ObjectBinding<ObservableList<OrderFactory>> binding = new ObjectBinding<ObservableList<OrderFactory>>() {
			...
			protected ObservableList<OrderFactory> computeValue() {
				...
				if (selectedItem != null
						&& selectedItem.getValue() instanceof SpaceObject) {
	...
	private void initAvailableLocationsTreeView() {
		...
		availableLocationsTreeView
				.setCellFactory(new PropertyOrToStringTreeCellFactory("name",
						null, String.class));
		...
		final ObjectBinding<TreeItem> contextualStellarSystemBinding = new ObjectBinding<TreeItem>() {
...

			{
				squadronProperty().addListener(new ChangeListener<Squadron>() {
					@Override
					public void changed(
							ObservableValue<? extends Squadron> arg0,
							Squadron arg1, Squadron arg2) {
						invalidate();
						if (arg1 != null) {
							arg1.contextualStellarSystemProperty()
									.removeListener(
											contextualStellarSystemListener);
						}
						if (arg2 != null) {
							arg2.contextualStellarSystemProperty().addListener(
									contextualStellarSystemListener);
						}
					}
				});
				...
				if (getSquadron() == null) {
					...
					for (Squadron squadronChild : stellarSystem.getSquadrons()) {
						if (squadronChild != getSquadron()) {
							squadronRoot.getChildren().add(
									new TreeItem(squadronChild));
						}
					}
					return root;
				}
		...
		availableLocationsTreeView.rootProperty().bind(
				contextualStellarSystemBinding);
		updateFilters(null);
	}
	...
	private void initShipListView() {
		ObjectBinding<ObservableList<Ship>> select = Bindings.select(
				squadronProperty(), "ships");
		shipsTableView.itemsProperty().bind(select);
		shipNameColumn
				.setCellValueFactory(new PropertyValueFactory<Ship, String>(
						"name"));
	}

	private void initSquadronInformation() {
		squadronSpeedSelectionComponent.squadronProperty().bind(
	...
	private void initSystemListView() {
		...
		systemListView
				.setCellFactory(new PropertyListCellFactory<StellarSystem>(
						"name", null));
	...
	public void onAvailableOrderClick(MouseEvent event) {
		if (event.getClickCount() >= 2 && getSquadron() != null) {
			...
			LOG.trace("Added Order " + order.getShortDescription() + " to "
					+ getSquadron().getName());
