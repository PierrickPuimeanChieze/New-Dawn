...
public class MoveToSpaceObjectOrder extends MoveOrder {
	...
	public void applyOrder() {
		LOG.trace("order [" + getShortDescription() + "] applied to squadron ["
				+ getSquadron().getName() + "]");
	...
	public ReadOnlyStringProperty shortDescriptionProperty() {
		if (shortDescriptionProperty == null) {
			shortDescriptionProperty = new SimpleStringProperty("Move To: "
	...
	public ReadOnlyStringProperty longDescriptionProperty() {
		if (longDescriptionProperty == null) {
			longDescriptionProperty = new SimpleStringProperty(
					"Following this order, the task group "
							+ getSquadron().getName() + " will try to move to "
							+ getDestination().getName());
		}
	...
	public boolean isOrderAccomplished() {
		return getDestination().getPositionX() == getSquadron().getPositionX()
				&& getDestination().getPositionY() == getSquadron()
						.getPositionY();
	}

	@Override
	public void finalizeOrder() {
		LOG.trace("[" + getShortDescription() + "] order for squadron ["
				+ getSquadron().getName() + "] finalized");
	...
	public void executeOrder(long incrementSize) {
		if (getSquadron().getDestination() != null) {
			...
			Point2D squadronPosition = new Point2D(
					getSquadron().getPositionX(), getSquadron().getPositionY());
			Point2D destinationPosition = new Point2D(getSquadron()
					.getDestination().getPositionX(), getSquadron()
			...
			// If the maximum traveled distance is enough to reach the
			// destination
			if (destinationDistance < traveledDistance) {
				// We move the squadron to the destination
				getSquadron().setPositionX(
						getSquadron().getDestination().getPositionX());
				getSquadron().setPositionY(
						getSquadron().getDestination().getPositionY());
				...
				Point2D newPositionForShip = ShipUtils
						.calculateIntermediateCoordinate(squadronPosition,
								destinationPosition, traveledDistance);
