...
public class ShipMovementController {
	...
	void moveTaskGroup(long second, Squadron taskGroup) {
		if (taskGroup.getDestination() != null) {
			...
			Point2D taskGroupPosition = new Point2D(taskGroup.getPositionX(),
					taskGroup.getPositionY());
			Point2D destinationPosition = new Point2D(taskGroup
			...
			if (destinationDistance < traveledDistance) {
				taskGroup.setPositionX(taskGroup.getDestination()
						.getPositionX());
				taskGroup.setPositionY(taskGroup.getDestination()
						.getPositionY());
			} else {
				Point2D newPositionForShip = ShipUtils
						.calculateIntermediateCoordinate(taskGroupPosition,
