...
public class TeamController {
    ...
    public void addMemberToTeam(Official newMember, FieldTeam team) {
        ...
        if (validateAdditionError.length > 0) {
            ...
            StringBuilder message = new StringBuilder("The member %s cannot be added to the team %s. Errors :");
            for (String string : validateAdditionError) {
                message.append(string);
                message.append('\n');
            }
            throw new PersonnelAssignmentException(message.toString(), newMember.
                    getName(), team.getName());
        } else {
    ...
    public boolean promotingTeamMemberToLeader(FieldTeam team, Official member) {
        ...
        if (validateAdditionError.length > 0) {
            StringBuilder message = new StringBuilder("The member %s cannot be promoted to leader for the team %s. Errors :");
            for (String string : validateAdditionError) {
                message.append(string);
                message.append('\n');
            }
    ...
    public String[] canPromoteTeamMember(Official wantedLeader, FieldTeam team) {
        ...
        if (!removeTeamMember(testTeam, wantedLeader)) {
            throw new PersonnelAssignmentException("the member %s is not a member of a the team %s", wantedLeader.
                    getName(), team.getName());
        }
